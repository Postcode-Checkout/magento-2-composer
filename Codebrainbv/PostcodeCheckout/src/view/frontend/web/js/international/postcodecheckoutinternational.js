(function () {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Listen for address form updates (customize this event for Magento 2)
        document.body.addEventListener('updatedAddressForm', function () {
            if (document.getElementById('checkout-addresses-step') &&
                document.getElementById('checkout-addresses-step').classList.contains('-current')) {
                pcm2_addLookup();
            }
        });
    });
    ...
    function pcm2_addLookup() {
        ...
        var sCountryId = countrySelect ? countrySelect.value : '';
        var sCountryCode = pcm2_getCountryCodeFromId(sCountryId);

        if (!pcm2_isSupportedCountry(sCountryCode)) return;

        // Does the form already have the fields
        if (!document.getElementById('pcm2_checkout-addresses-step_search_wrapper')) {
            pcm2ConsoleLogs('Adding address lookup fields to the checkout form');
            pcm2_hideForm('checkout-addresses-step');
        } else {
            return;
        }

        if (window.pcm2_config.user_logged) {
            var dom = pcm2_getDomElements();
            var sAddress1 = dom.address_1 ? dom.address_1.value : '';
            var sPostcode = dom.postcode ? dom.postcode.value : '';
            var sCity = dom.city ? dom.city.value : '';
        ...
        var manualBtn = document.getElementById('pcm2_checkout-addresses-step_manualbtn');
        var autoBtn = document.getElementById('pcm2_checkout-addresses-step_autobtn');
        ...
        var sIso3Code = pcm2_convertIso2ToIso3(sCountryCode);

        var sSearchId = document.getElementById('pcm2_checkout-addresses-step_search');
        if (sSearchId && window.PostcodeNl && window.PostcodeNl.AutocompleteAddress) {
            var pcex4presta_Autocomplete = new window.PostcodeNl.AutocompleteAddress(sSearchId, {
                autocompleteUrl: "/module/cb_postcode_checkout/suggestions?type=autocomplete",
                addressDetailsUrl: "/module/cb_postcode_checkout/details?type=address",
                autoFocus: true,
                autoSelect: true,
                context: sIso3Code
            });

            sSearchId.addEventListener('addressSelected', function (event) {
                if (event.detail.precision === 'Address') {
                    pcex4presta_Autocomplete.getDetails(event.detail.context, function (result) {
                        pcm2ConsoleLogs(result);

                        var dom = pcm2_getDomElements();
                        if (result.address.street && result.address.buildingNumber && result.address.locality) {
                            if (window.pcm2_config.address_2 == 2) {
                                dom.address_1.value = result.address.street;
                                dom.address_2.value = result.address.buildingNumber + ' ' + (result.address.buildingNumberAddition || '');
                            } else if (window.pcm2_config.address_2 == 1) {
                                dom.address_1.value = result.address.street + ' ' + result.address.buildingNumber;
                                dom.address_2.value = result.address.buildingNumberAddition || '';
                            } else {
                                dom.address_1.value = result.address.street + ' ' + result.address.buildingNumber + ' ' + (result.address.buildingNumberAddition || '');
                            }

                            dom.postcode.value = pcm2_formatPostcode(result.address.postcode, sCountryCode);
                            dom.city.value = result.address.locality;
    ...
    function pcm2_updatePreview(sFormId) {
        var address1 = document.querySelector('[name="address_1"]') ? document.querySelector('[name="address_1"]').value : '';
        var address2 = document.querySelector('[name="address_2"]') ? document.querySelector('[name="address_2"]').value : '';
        var postcode = document.querySelector('[name="postcode"]') ? document.querySelector('[name="postcode"]').value : '';
        var city = document.querySelector('[name="city"]') ? document.querySelector('[name="city"]').value : '';
        var resultDiv = document.getElementById('pcm2_' + sFormId + '_result');
        if (resultDiv) {
            resultDiv.innerHTML = address1 + ' ' + address2 + '<br>' + postcode + ' ' + city;
        }
    }
    ...
    function pcm2_getDomElements() {
        return {
            address_1: document.querySelector('[name="address_1"]'),
            address_2: document.querySelector('[name="address_2"]'),
            postcode: document.querySelector('[name="postcode"]'),
            city: document.querySelector('[name="city"]'),
            country: document.querySelector('[name="id_country"]')
        };
    }
    ...
    // (Removed malformed duplicate function definition and orphaned else block)
